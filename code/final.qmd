...
```{r message=FALSE}
# Filter just Swift Parrot rows
swift_data <- filter(nest_boxes_clean, species == "Swift Parrot")

model1 <- lm(
  count ~ 1,  # or add edge_distance if grouped that way
  data = filter(nest_boxes_counts, species == "Swift Parrot")
)

names(model0) <- unique(nest_boxes_clean$species)

# Model 1: all predictors
model1 <- glm(
  presence ~ edge_distance + season,
  data = swift_data,
  family = "binomial"
)
